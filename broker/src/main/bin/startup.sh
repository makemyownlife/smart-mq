...
if [ -f $base/bin/smartmq.pid ] ; then
	echo "found smartmq.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
if [ -n "$str" ]; then
  ...
  # For G1
  JAVA_OPTS="-server -Xms2g -Xmx2g -XX:+UseG1GC -XX:MaxGCPauseMillis=250 -XX:+UseGCOverheadLimit -XX:+ExplicitGCInvokesConcurrent $JAVA_OPTS"
else
